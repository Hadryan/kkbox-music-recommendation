...
# merge data
member = pd.read_csv(DATA_DIR + 'members_gbdt.csv')

train = train.merge(member, on='msno', how='left')
test = test.merge(member, on='msno', how='left')

print('Member data loaded.')

del member
gc.collect()

member_add = pd.read_csv(DATA_DIR + 'members_add.csv')
...
print('Member add data loaded.')

del member_add
gc.collect()

song = pd.read_csv(DATA_DIR + 'songs_gbdt.csv')
...
#     train['before_source_type'] == train['source_type']) * 1.0
# test['before_type_same'] = (
#     test['before_source_type'] == test['source_type']) * 1.0

# train['after_type_same'] = (
#     train['after_source_type'] == train['source_type']) * 1.0
# test['after_type_same'] = (test['after_source_type']
#                            == test['source_type']) * 1.0

# train['before_artist_same'] = (
#     train['before_artist_name'] == train['artist_name']) * 1.0
# test['before_artist_same'] = (
#     test['before_artist_name'] == test['artist_name']) * 1.0

# train['after_artist_same'] = (
#     train['after_artist_name'] == train['artist_name']) * 1.0
# test['after_artist_same'] = (
#     test['after_artist_name'] == test['artist_name']) * 1.0
'''
...
train['msno_upper_time'] = train['msno_timestamp_mean'] + \
    train['msno_timestamp_std']
test['msno_upper_time'] = test['msno_timestamp_mean'] + \
    test['msno_timestamp_std']

train['msno_lower_time'] = train['msno_timestamp_mean'] - \
    train['msno_timestamp_std']
test['msno_lower_time'] = test['msno_timestamp_mean'] - \
    test['msno_timestamp_std']

# song-side features
train['song_upper_time'] = train['song_timestamp_mean'] + \
    train['song_timestamp_std']
test['song_upper_time'] = test['song_timestamp_mean'] + \
    test['song_timestamp_std']

train['song_lower_time'] = train['song_timestamp_mean'] - \
    train['song_timestamp_std']
test['song_lower_time'] = test['song_timestamp_mean'] - \
    test['song_timestamp_std']
...
# feature selection
feat_importance = pd.read_csv(CONFIG_DIR + 'lgb_feature_importance.csv')
...
drop_col = [x for x in drop_col if x in train.columns]
...
para = pd.read_csv(CONFIG_DIR +
                   'lgb_record.csv').sort_values(by='val_auc', ascending=False)
for i in range(1):
    params = {
    ...
    gbm = lgb.train(params, train_data, num_round,
                    valid_sets=train_data, verbose_eval=100)

    val_auc = para['val_auc'].values[i]
    print('Model training done. Validation AUC: %.5f' % val_auc)

    feature_importance = pd.DataFrame({'name': gbm.feature_name(
    ), 'importance': gbm.feature_importance()}).sort_values(by='importance', ascending=False)
    feature_importance.to_csv(
        OUTPUT_DIR + 'feat_importance_for_test.csv', index=False)

    flag = np.random.randint(0, 65536)

    test_pred = gbm.predict(test)
    test_sub = pd.DataFrame({'id': test_id, 'target': test_pred})
    test_sub.to_csv(OUTPUT_DIR + 'lgb_%.5f_%d.csv.gz' % (val_auc, flag), index=False,
                    compression='gzip')
