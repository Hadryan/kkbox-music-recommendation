...
# load the data
tr = pd.read_csv(INPUT_DIR + 'train_id_cnt_svd.csv')
te = pd.read_csv(INPUT_DIR + 'test_id_cnt_svd.csv')
mem = pd.read_csv(INPUT_DIR + 'members_id_cnt_svd.csv')
song = pd.read_csv(INPUT_DIR + 'songs_id_cnt_isrc_svd.csv')
...
for window_size in window_sizes:
    ...
    for i in range(len(concat)):
        msno_before_cnt[i], msno_after_cnt[i] = get_window_cnt(
            msno_list, i, window_size)
        song_before_cnt[i], song_after_cnt[i] = get_window_cnt(
            song_list, i, window_size)
...
def timestamp_map(x):
    if x < 7377418:
        x = (x - 0.0) / (7377417.0 - 0.0) * \
            (1484236800.0 - 1471190400.0) + 1471190400.0
    else:
        x = (x - 7377417.0) / (9934207.0 - 7377417.0) * \
            (1488211200.0 - 1484236800.0) + 1484236800.0
...
tr.to_csv(OUTPUT_DIR + 'train_id_cnt_svd_stamp.csv', index=False)
te.to_csv(OUTPUT_DIR + 'test_id_cnt_svd_stamp.csv', index=False)
mem.to_csv(OUTPUT_DIR + 'members_id_cnt_svd_stamp.csv', index=False)
song.to_csv(OUTPUT_DIR + 'songs_id_cnt_isrc_svd_stamp.csv', index=False)
