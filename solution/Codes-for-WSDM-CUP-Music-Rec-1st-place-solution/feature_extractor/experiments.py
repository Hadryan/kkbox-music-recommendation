...
tr = pd.read_csv(INPUT_DIR + 'train_part.csv')
te = pd.read_csv(INPUT_DIR + 'test.csv')
song = pd.read_csv(INPUT_DIR + 'songs_nn.csv')
...
data_avg = data_avg.rename_axis(None)
mem_add = mem_add.rename_axis(None)
...
data_std = data_std.rename_axis(None)
mem_add = mem_add.merge(data_std, on='msno', how='left')

artist_msno = concat[['msno', 'artist_name']].groupby(
    'msno').apply(lambda x: len(set(x['artist_name'].values)))
...
language_prob = language_prob.rename_axis(None)
mem_add = mem_add.merge(language_prob, on='msno', how='left')

mem_add.to_csv(OUTPUT_DIR + 'members_add.csv', index=False)
...
msno_cnt = concat[['msno', 'song_id']].groupby(
    'msno').count().to_dict()['song_id']
concat['msno_cnt'] = concat['msno'].apply(lambda x: msno_cnt[x])
for feat in col:
    concat['msno_'+feat+'_prob'] = concat['msno_' +
                                          feat+'_cnt'] / concat['msno_cnt']
...
song_cnt = concat[['msno', 'song_id']].groupby(
    'song_id').count().to_dict()['msno']
concat['song_cnt'] = concat['song_id'].apply(lambda x: song_cnt[x])

for col in cols:
    concat['song_'+col+'_prob'] = concat['song_' +
                                         col+'_cnt'] / concat['song_cnt']
...
result[:len(tr)].to_csv(OUTPUT_DIR + 'train_part_add.csv', index=False)
result[len(tr):].to_csv(OUTPUT_DIR + 'test_add.csv', index=False)
