...
# load the data
tr = pd.read_csv(INPUT_DIR + 'train_id_cnt.csv')
te = pd.read_csv(INPUT_DIR + 'test_id_cnt.csv')
member = pd.read_csv(INPUT_DIR + 'members_id_cnt.csv')
song = pd.read_csv(INPUT_DIR + 'songs_id_cnt_isrc.csv')
...
members_topics.columns = ['member_component_%d' %
                          i for i in range(n_component)]
...
concat = concat.merge(song[['song_id', 'artist_name']],
                      on='song_id', how='left')
...
members_topics = pd.DataFrame(u[:, ::-1])
members_topics.columns = ['member_artist_component_%d' %
                          i for i in range(n_component)]
members_topics['msno'] = range(member_cnt)
...
for i in range(len(tr)):
    ...
    train_dot[i, 0] = np.dot(member_embeddings[msno_idx], np.dot(
        s_song, song_embeddings[song_idx]))
    train_dot[i, 1] = np.dot(member_artist_embeddings[msno_idx], np.dot(
        s_artist, song_artist_embeddings[song_idx]))

for i in range(len(te)):
    msno_idx = te['msno'].values[i]
    song_idx = te['song_id'].values[i]

    test_dot[i, 0] = np.dot(member_embeddings[msno_idx],
                            np.dot(s_song, song_embeddings[song_idx]))
    test_dot[i, 1] = np.dot(member_artist_embeddings[msno_idx], np.dot(
        s_artist, song_artist_embeddings[song_idx]))
...
# write to files
tr.to_csv(OUTPUT_DIR + 'train_id_cnt_svd.csv', index=False)
te.to_csv(OUTPUT_DIR + 'test_id_cnt_svd.csv', index=False)
member.to_csv(OUTPUT_DIR + 'members_id_cnt_svd.csv', index=False)
song.to_csv(OUTPUT_DIR + 'songs_id_cnt_isrc_svd.csv', index=False)
