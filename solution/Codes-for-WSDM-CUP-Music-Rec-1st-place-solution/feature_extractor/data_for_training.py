...
# load the data
train = pd.read_csv(INPUT_DIR + 'train_id_cnt_svd_stamp_before_after.csv')
test = pd.read_csv(INPUT_DIR + 'test_id_cnt_svd_stamp_before_after.csv')
member = pd.read_csv(INPUT_DIR + 'members_id_cnt_svd_stamp.csv')
song = pd.read_csv(INPUT_DIR + 'songs_id_cnt_isrc_svd_stamp.csv')

# prepare data for train / test
train.to_csv(OUTPUT_DIR + 'train.csv', index=False, float_format='%.6f')
test.to_csv(OUTPUT_DIR + 'test.csv', index=False, float_format='%.6f')
...
train.to_csv(OUTPUT_DIR + 'train_part.csv', index=False, float_format='%.6f')

# prepare data for member / song for GBDT
member.to_csv(OUTPUT_DIR + 'members_gbdt.csv', index=False)

columns = ['composer', 'lyricist', 'language',
...
song.to_csv(OUTPUT_DIR + 'songs_gbdt.csv', index=False)
...
member['msno_timestamp_std'].fillna(
    np.nanmin(member['msno_timestamp_std']), inplace=True)
member.to_csv(OUTPUT_DIR + 'members_nn.csv', index=False)
...
song.to_csv(OUTPUT_DIR + 'songs_nn.csv', index=False)
