...
songs_extra = pd.read_csv(INPUT_DIR + 'song_extra_info.csv')
...
songs_extra['appeared'] = songs_extra['song_id'].apply(
    lambda x: True if x in song_id_set else False)
...
members['appeared'] = members['msno'].apply(
    lambda x: True if x in msno_set else False)
...
genre_encoder.fit((songs.first_genre_id.append(
    songs.second_genre_id)).append(songs.third_genre_id))
...
# save files
members.to_csv(OUTPUT_DIR + 'members_id.csv', index=False)
songs.to_csv(OUTPUT_DIR + 'songs_id.csv', index=False)
songs_extra.to_csv(OUTPUT_DIR + 'songs_extra_id.csv', index=False)
train.to_csv(OUTPUT_DIR + 'train_id.csv', index=False)
test.to_csv(OUTPUT_DIR + 'test_id.csv', index=False)
