...
# load data
tr = pd.read_csv(INPUT_DIR + 'train_id_cnt_svd_stamp.csv')
te = pd.read_csv(INPUT_DIR + 'test_id_cnt_svd_stamp.csv')
...
for i in range(len(concat)):
    ...
    if(song_dict[msno] == None):
        before_data[i] = concat[['song_id', 'source_type',
                                 'source_screen_name', 'timestamp']].values[i]
...
for i in range(len(concat))[::-1]:
    ...
    if(song_dict[msno] == None):
        after_data[i] = concat[['song_id', 'source_type',
                                'source_screen_name', 'timestamp']].values[i]
...
tr.to_csv(OUTPUT_DIR + 'train_id_cnt_svd_stamp_before_after.csv', index=False)
te.to_csv(OUTPUT_DIR + 'test_id_cnt_svd_stamp_before_after.csv', index=False)
