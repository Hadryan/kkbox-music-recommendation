...
train = pd.read_csv(INPUT_DIR + 'train_id.csv')
test = pd.read_csv(INPUT_DIR + 'test_id.csv')
member = pd.read_csv(INPUT_DIR + 'members_id.csv')
song_origin = pd.read_csv(INPUT_DIR + 'songs_id.csv')
song_extra = pd.read_csv(INPUT_DIR + 'songs_extra_id.csv')
...
song['artist_song_cnt'] = song['artist_name'].apply(
    lambda x: artist_song_cnt[x] if not np.isnan(x) else np.nan)

composer_song_cnt = song.groupby(by='composer').count()['song_id'].to_dict()
composer_song_cnt[0] = np.nan
song['composer_song_cnt'] = song['composer'].apply(
    lambda x: composer_song_cnt[x] if not np.isnan(x) else np.nan)

lyricist_song_cnt = song.groupby(by='lyricist').count()['song_id'].to_dict()
lyricist_song_cnt[0] = np.nan
song['lyricist_song_cnt'] = song['lyricist'].apply(
    lambda x: lyricist_song_cnt[x] if not np.isnan(x) else np.nan)

genre_song_cnt = song.groupby(by='first_genre_id').count()['song_id'].to_dict()
genre_song_cnt[0] = np.nan
song['genre_song_cnt'] = song['first_genre_id'].apply(
    lambda x: genre_song_cnt[x] if not np.isnan(x) else np.nan)

data = data.merge(song, on='song_id', how='left')

song_rec_cnt = data.groupby(by='song_id').count()['msno'].to_dict()
song['song_rec_cnt'] = song['song_id'].apply(
    lambda x: song_rec_cnt[x] if not np.isnan(x) else np.nan)

artist_rec_cnt = data.groupby(by='artist_name').count()['msno'].to_dict()
song['artist_rec_cnt'] = song['artist_name'].apply(
    lambda x: artist_rec_cnt[x] if not np.isnan(x) else np.nan)

composer_rec_cnt = data.groupby(by='composer').count()['msno'].to_dict()
composer_rec_cnt[0] = np.nan
song['composer_rec_cnt'] = song['composer'].apply(
    lambda x: composer_rec_cnt[x] if not np.isnan(x) else np.nan)

lyricist_rec_cnt = data.groupby(by='lyricist').count()['msno'].to_dict()
lyricist_rec_cnt[0] = np.nan
song['lyricist_rec_cnt'] = song['lyricist'].apply(
    lambda x: lyricist_rec_cnt[x] if not np.isnan(x) else np.nan)

genre_rec_cnt = data.groupby(by='first_genre_id').count()['msno'].to_dict()
genre_rec_cnt[0] = np.nan
song['genre_rec_cnt'] = song['first_genre_id'].apply(
    lambda x: genre_rec_cnt[x] if not np.isnan(x) else np.nan)
...
for feat in dummy_feat:
    ...
    feat_dummies.columns = ['msno_%s_' % feat + '%s' %
                            col for col in feat_dummies.columns]
    feat_dummies['msno'] = concat['msno'].values
    feat_dummies = feat_dummies.groupby('msno').mean()
    feat_dummies['msno'] = feat_dummies.index
    member = member.rename_axis(None)
    feat_dummies = feat_dummies.rename_axis(None)
...
member.to_csv(OUTPUT_DIR + 'members_id_cnt.csv', index=False)
...
song.to_csv(OUTPUT_DIR + 'songs_id_cnt.csv', index=False)

train.to_csv(OUTPUT_DIR + 'train_id_cnt.csv', index=False)
test.to_csv(OUTPUT_DIR + 'test_id_cnt.csv', index=False)
